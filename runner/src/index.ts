...
const ID_FILE =
  process.env.AGENTMESH_RUNNER_ID_FILE || new URL('../data/identity.json', import.meta.url).pathname;
...
function parseEnrollCode(rawCode: string): EnrollCodePayload {
  const raw = rawCode.trim();
  if (!raw) throw new Error('empty enroll code');
  let decoded = '';
  try {
  ...
  return payload as EnrollCodePayload;
}
...
async function registerRunner(): Promise<RunnerIdentity> {
  ...
  if (!j?.runnerId || !j?.runnerToken) throw new Error(`bad register response: ${JSON.stringify(j)}`);
  const id: RunnerIdentity = { runnerId: j.runnerId, runnerToken: j.runnerToken, createdAt: Date.now() };
...
async function enrollRunner(enrollCode: string): Promise<RunnerIdentity> {
  ...
  const j: any = await res.json();
  if (!j?.runnerId || !j?.runnerToken) throw new Error(`bad enroll response: ${JSON.stringify(j)}`);
    ...
    try {
      ...
      const flushOutgoing = () => {
        ...
        while (outQueue.length && sent < WS_FLUSH_BATCH) {
          if (ws.bufferedAmount > WS_BACKPRESSURE_LIMIT) break;
          ...
          ws.send(frame, { binary: true }, (err) => {
            if (err) console.error('[runner] ws send(binary) failed', err?.message || err);
          });
          sent += 1;
        }
      ...
      const enqueuePtyData = (sessionId: string, data: string) => {
        if (!data) return;
        ...
        for (let i = 0; i < raw.length; i += PTY_CHUNK_BYTES) {
          ...
          if (queuedBytes > counters.queueHighWatermarkBytes) counters.queueHighWatermarkBytes = queuedBytes;
        }
        ...
        if (!flushTimer) flushTimer = setInterval(flushOutgoing, 12);
        flushOutgoing();
      };

      const snapshotActiveSessions = () =>
        Array.from(sessions.entries()).map(([sessionId, ps]) => ({
          sessionId,
      ...
      ws.on('message', async (data, isBinary) => {
        if (isBinary) {
          const parsed = decodeBinaryFrame(rawDataToBuffer(data as any));
          if (!parsed) {
          ...
          if (parsed.kind === BIN_MSG_PTY_INPUT) {
            ...
            if (!ps) return;
            if (parsed.payload.length) ps.pty.write(parsed.payload.toString('utf8'));
            return;
          }
        ...
        if (msg?.type === 'start_session') {
          ...
          try {
            ...
            if (sessions.has(sessionId)) {
              ws.send(
                JSON.stringify({ type: 'start_session_result', ok: false, sessionId, error: 'session already exists' }),
              );
              return;
            }
        ...
        if (msg?.type === 'pty_input') {
          ...
          const ps = sessions.get(sessionId);
          if (ps && input) ps.pty.write(input);
          return;
        }
