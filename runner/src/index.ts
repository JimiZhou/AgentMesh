...
function parseEnrollCode(rawCode: string): EnrollCodePayload {
  ...
  if (!payload || typeof payload !== 'object' || typeof (payload as EnrollCodePayload).token !== 'string') {
    throw new Error('invalid enroll code payload: token missing');
  }

  const exp = Number((payload as EnrollCodePayload).exp || 0);
...
function resolveProjectPath(projectPath: unknown): string {
...
function snapshotActiveSessions(sessions: Map<string, AcpRunnerSession>): ActiveSessionSummary[] {
...
async function connectLoop() {
  ...
  let attempt = 0;
  const sessions = new Map<string, AcpRunnerSession>();
  ...
  const emitFromSession = (message: Record<string, unknown>) => {
    if (message.type === 'acp_permission_request') counters.permissionRequests += 1;
